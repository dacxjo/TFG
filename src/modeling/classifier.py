...
class MolecularSubtypeClassifierV2(L.LightningModule):
    ...
    def __init__(
        ...
        # Init
        self.backbone_mlo = backbone_mlo
        self.backbone_cc = backbone_cc
        self.num_classes = num_classes
        self.learning_rate = learning_rate
        self.classification_mode = classification_mode
        self.class_weights = class_weights
        ...
        feature_dim = self.backbone_mlo.embed_dim
        ...
        self.save_hyperparameters(ignore=["backbone_mlo", "backbone_cc"])
    ...
    def forward(self, x):
        ...
        """
        mlo_img, cc_img = x
        mlo_features = self.backbone_mlo(mlo_img)
        cc_features = self.backbone_cc(cc_img)

        if mlo_features.ndim > 2:
            mlo_features = torch.flatten(mlo_features, start_dim=1)
        if cc_features.ndim > 2:
            cc_features = torch.flatten(cc_features, start_dim=1)

        combined = torch.cat((mlo_features, cc_features), dim=1)
    ...
    def _shared_step(self, batch, batch_idx, stage):
        ...
        :return: Loss value.
        """
        (mlo_img, cc_img), y = batch
        logits = self((mlo_img, cc_img))
        loss = self.criterion(logits, y)
